...
const bindingPath = path.join(bindingDir, 'zt-block-system-shortcuts.node');
...
function ensureBinding() {
  ...
  for (const napiVer of napiVersions) {
    ...
    process.stdout.write(`zt-block-system-shortcuts: downloading prebuilt binary from ${downloadUrl}\n`);
    ...
    process.stderr.write(`zt-block-system-shortcuts: prebuilt binary not available for ${platform}-${arch} (${downloadUrl})\n`);
  }

  throw new Error(
    `zt-block-system-shortcuts: no prebuilt binary available for ${platform}-${arch} v${require('./package.json').version}.\n` +
...
const addon = bindings('zt-block-system-shortcuts');
